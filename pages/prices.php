...
// Atualizar dados do serviço (incluindo preços)
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['save_row'])) {
  ...
  if(!in_array($cat,$cats,true))           { $err = 'Categoria inválida.'; }
  elseif($nome==='')                       { $err = 'Informe o nome.'; }
  elseif(!array_key_exists($uni,$units))   { $err = 'Unidade inválida.'; }
  else {
    $st = $pdo->prepare("UPDATE services_all SET categoria=?, nome=?, unidade=?, preco_final=?, preco_corporativo=?, observacao=?, ativo=? WHERE id=?");
    $st->execute([$cat,$nome,$uni,$pf,$pc,$obs,$ativo,$id]);
...
$params = [];
if (in_array($f_cat,$cats,true)) { $sql .= " AND categoria=?"; $params[] = $f_cat; }
...
// Agrupa para exibir por categoria
$grouped = [];
foreach ($rows as $r) { $grouped[$r['categoria']][] = $r; }
?>
<div class="container py-4">
  <h3>Preços e Catálogo de Serviços</h3>
  <p class="text-muted">Adicione e mantenha o catálogo de serviços a partir desta tela. Os valores serão utilizados para gerar orçamentos e ordens de serviço.</p>

  <?php if($err): ?><div class="alert alert-danger"><?=h($err)?></div><?php endif; ?>
  <?php if($msg): ?><div class="alert alert-success"><?=h($msg)?></div><?php endif; ?>

  <form class="row g-2 mb-3" method="get">
    <input type="hidden" name="route" value="prices">
    <div class="col-md-3">
    ...
    <div class="col-md-5">
      <input name="q" class="form-control" placeholder="Buscar por nome ou observação" value="<?=h($q)?>">
    </div>
    <div class="col-md-2">
      <button class="btn btn-outline-secondary w-100">Filtrar</button>
    </div>
    <div class="col-md-2">
      <a class="btn btn-primary w-100" href="/?route=services-new">Novo serviço</a>
    </div>
  </form>

  <?php if(!$rows): ?>
    <div class="alert alert-warning">Nenhum serviço encontrado. Utilize o botão &ldquo;Novo serviço&rdquo; para cadastrar.</div>
  <?php endif; ?>
            ...
            <tbody>
              ...
              <tr>
                <form method="post" class="row g-1">
                  <td>
                    <input name="nome" class="form-control form-control-sm" value="<?=h($r['nome'])?>">
